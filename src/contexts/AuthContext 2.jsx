...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(async (empNum, options = {}) => {
    ...
    if (staff && forceRole !== 'trainee') {
      ...
      if (traineeId && !forceRole && staff.role !== 'admin' && staff.role !== 'owner') {
        return { needsRolePicker: true, staffInfo: { ...staff, empNum }, traineeId }
      } else {
  ...
  const logout = useCallback(async () => {
    ...
    if (isTrainee) {
      const traineeId = sessionStorage.getItem('traineeId')
      const sessionToken = sessionStorage.getItem('sessionToken')
      logoutSession(traineeId, sessionToken)
      sessionStorage.removeItem('sessionToken')
      sessionStorage.removeItem('traineeId')
      sessionStorage.removeItem('currentUser')
      currentSessionTokenRef.current = null
